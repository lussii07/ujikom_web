...
<div class="container mx-auto px-6 py-8">
    <h1 class="text-3xl font-semibold text-gray-100 mb-6">Data Galery</h1>
        ...
        <table class="min-w-full table-auto bg-gray-800 text-gray-100">
            <thead class="bg-gray-700 text-gray-300">
                <tr>
                    ...
                    <th class="py-3 px-4 text-left uppercase font-semibold text-sm">Post ID</th>
                    <th class="py-3 px-4 text-left uppercase font-semibold text-sm">Position</th>
            ...
            <tbody class="divide-y divide-gray-600">
                @foreach($galeries as $index => $gallery)
                    <tr class="hover:bg-gray-700 transition duration-200 {{ $index % 2 === 0 ? 'bg-gray-800' : 'bg-gray-750' }}">
                        <td class="py-4 px-4">{{ $gallery->id }}</td>
                        <td class="py-4 px-4">{{ $gallery->post_id }}</td>
                        <td class="py-4 px-4">{{ $gallery->position }}</td>
                        <td class="py-4 px-4">{{ $gallery->status }}</td>
                        <td class="py-4 px-4 text-center">
                            <a href="{{ route('admin.galery.edit', $gallery) }}" 
                               class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded text-sm mr-2 transition">
                                <i class="fas fa-edit"></i> Edit
                            </a>
                            <form action="{{ route('admin.galery.destroy', $gallery) }}" 
